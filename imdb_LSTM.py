...
# 模型训练
def train(model, train_data,test_data, vocab, epoch=2):
    print('train model')
    ...
    for epoch_num in tqdm(range(epoch)):
        ...
        for idx, (text, label) in enumerate(tqdm(train_data)):
            ...
            print("epoch:",epoch_num+1," ,idx:",idx)
        ...
        print("epoch:",epoch_num+1, "avg_loss:", avg_loss, " train_avg_acc:,", avg_acc)
        # 保存训练完成后的模型参数
...
def test(model, test_data, vocab):
    ...
    for idx, (text, label) in enumerate(tqdm(test_data)):
        ...
        pred = model(train_x)
        if label==0:
            avg_acc_0 += accuracy(pred, train_y)
        if label==1:
            avg_acc_1 += accuracy(pred, train_y)
    avg_acc_0 = avg_acc_0 / len(test_data)
    avg_acc_1 = avg_acc_1 / len(test_data)

    print('Accuracy : %d %%' % (100 * avg_acc))
    print('Accuracy of class0 : %2d %%' % (100 * avg_acc_0))
    print('Accuracy of class1 : %2d %%' % (100 * avg_acc_1))
...
def main():
    ...
    model, train_process = train(model=model, train_data=train_loader,test_data=test_loader, vocab=vocab, epoch=1)
